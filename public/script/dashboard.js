...
function formatState(state) {
    ...
    const $state = $(
        `<span><img src="/img/assets/${tradeSymbol}.svg" alt="${state.text}" style="width: 20px; height: 20px; vertical-align: middle;" /> ${state.text}</span>`
    );
    return $state;
}
...
function searchCurrencyValue() {
    const selectedAsset = $('#currency').val();
    const assetDisplay = $('#asset-display'); // The area where the amount is displayed
    assetDisplay.text(selectedAsset); // Update the display area with the selected asset
}

$('#currency').on('change', searchCurrencyValue);
        ...
        if (tradeCurrency) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#buydisplayasset').text(response.tradeSymbol);
                }})};
